...
#
# NOTE: "find" and "improve" are deliberately NOT here even though they might
# look safe at a glance — both do real filesystem I/O (find enumerates paths;
# improve reads arbitrary file contents and sends them to an AI provider), so
# they belong in SYSTEM_MODULES only. A module in two sets isn't just harmless
# duplication: allowed_modules() checks SAFE/AI before SYSTEM, so double-
# classifying a SYSTEM module as SAFE or AI silently un-blocks it in server
# mode. Verified 2026-07-14: this exact bug existed for both "find" (SAFE+
# SYSTEM) and "improve" (AI+SYSTEM) until this pass.
...
AI_MODULES: frozenset[str] = frozenset({
    ...
    "aiconfig", "imagegen", "learner", "qa", "agent", "chain",
})
